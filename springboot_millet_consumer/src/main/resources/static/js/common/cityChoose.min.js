...
Vcity.CitySelector.prototype = {
  ...
  createWarp:function(){
    ...
    var inputDiffer = this.input.offsetHeight;
    ...
    // 设置点击文档隐藏弹出的城市选择框
    /*Vcity._m.on(that.input.parentNode, 'mouseout', function (event) {
    ...
    Vcity._m.on(that.input.parentNode, 'mouseleave', function (event) {
      event = Vcity._m.getEvent(event);
      var target = Vcity._m.getTarget(event);
      if(target == that.input) return false;
      //console.log(target.className);
    ...
    div.style.right = 26 + 'px';
    div.style.top = inputDiffer + 'px';
